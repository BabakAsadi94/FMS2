...
class ConditionalFlowMatcherBoth:
    def __init__(self, sigma: Union[float, int] = 0.0):
        self.sigma = sigma
    ...
    def sample_location_and_conditional_flow(self, x0, x1, y0, y1, t=None, return_noise=False):
        if t is None:
            t = torch.rand(x0.shape[0]).type_as(x0)
        assert len(t) == x0.shape[0], "t has to have batch size dimension"

        eps = self.sample_noise_like(x0)
        eps_y = self.sample_noise_like(y0)  # Independent noise for masks
        xt = self.sample_xt(x0, x1, t, eps)
        yt = self.sample_xt(y0, y1, t, eps_y)
        ut = self.compute_conditional_flow(x0, x1, t, xt)
        ut_y = self.compute_conditional_flow(y0, y1, t, yt)
        if return_noise:
            return t, xt, ut, yt, ut_y, eps, eps_y
        else:
            return t, xt, ut, yt, ut_y

    def compute_lambda(self, t):
        sigma_t = self.compute_sigma_t(t)
        return 2 * sigma_t / (self.sigma**2 + 1e-8)

class DeepfakeConditionalFlowMatcher:
    def __init__(self, sigma: Union[float, int] = 0.0, noise_ratio: float = 0.3):
        self.sigma = sigma
        self.noise_ratio = noise_ratio  # rather than 0~1. t should be noise_ratio ~ 1
    ...
    def sample_location_and_conditional_flow(self, x0, x1, t=None, return_noise=False):
        if t is None:
            t = torch.rand(x0.shape[0]).type_as(x0) * (1 - self.noise_ratio) + self.noise_ratio
        assert len(t) == x0.shape[0], "t has to have batch size dimension"
...
class RectifiedConditionalFlowMatcher:
    def __init__(self, sigma=0.0, schedule="t2", alpha=2.0):
        self.sigma = sigma
    ...
    def phi_prime(self, t):
        if self.schedule == "t2":
            ...
            return self.alpha * (t.clamp(0,1) ** (self.alpha - 1))
        elif self.schedule == "linear":
    ...
    def sample_location_and_conditional_flow(self, x0, x1):
        ...
        phi_t = self.phi(t_expand)
        xt = (1 - phi_t) * x0 + phi_t * x1

        direction = (x1 - x0)
        ut = self.phi_prime(t_expand) * direction  # rectified velocity
        return t, xt, ut
...
class RectifiedConditionalFlowMatcher_v2:
    def __init__(self, sigma=0.2, schedule="t2", alpha=2.0):
        self.sigma = sigma
        self.schedule = schedule
        self.alpha = alpha
    ...
    def sample_location_and_conditional_flow(self, x0, x1):
        ...
        t_expand = t.view(-1, *([1]*(x0.ndim-1)))
        phi_t = self.phi(t_expand)
        # stochastic interpolant noise scale: max at mid-time, 0 at ends
        # works well in practice:
        sigma_t = self.sigma * torch.sqrt(torch.clamp(phi_t*(1 - phi_t), min=0.0))
        eps = torch.randn_like(x0)

        xt = (1 - phi_t) * x0 + phi_t * x1 + sigma_t * eps
        ut = self.phi_prime(t_expand) * (x1 - x0)  # rectified velocity
        return t, xt, ut

class AsymmetricNoiseConditionalFlowMatcher:
    """
    A novel flow matcher designed for Image-to-Mask translation.
    It injects noise asymmetrically: high noise near the image (t=0) to destroy 
    irrelevant textures, but zero noise near the mask (t=1) to ensure crisp, 
    deterministic boundaries for thin structures.
    """
    def __init__(self, sigma=0.5, schedule="t2", alpha=2.0):
        self.sigma = sigma
        self.schedule = schedule
        self.alpha = alpha
    ...
    def sample_location_and_conditional_flow(self, x0, x1):
        ...
        # Asymmetric noise: peaks at t=0, decays to 0 at t=1
        # This forces the model to learn robust features early on, 
        # but allows deterministic refinement of thin cracks at the end.
        sigma_t = self.sigma * (1 - phi_t)
        eps = torch.randn_like(x0)

        xt = (1 - phi_t) * x0 + phi_t * x1 + sigma_t * eps
        
        # The velocity must account for the deterministic path. 
        # Note: In standard stochastic interpolants, the score term is handled separately,
        # but for simple flow matching, we target the deterministic velocity.
        ut = self.phi_prime(t_expand) * (x1 - x0)
        
        return t, xt, ut


class FrequencyModulatedConditionalFlowMatcher:
    """
    A novel flow matcher that blends low frequencies first, then high frequencies.
    For crack segmentation, the general location (low freq) is found early, 
    and the sharp edges (high freq) are refined late in the flow.
    """
    def __init__(self, blur_sigma_max=5.0):
    ...
    def sample_location_and_conditional_flow(self, x0, x1):
        ...
        xt_linear = (1 - t_expand) * x0 + t_expand * x1
        
        # We don't actually apply Gaussian blur in the tensor directly here because 
        # batched variable-sigma blur is complex in pure PyTorch without loops.
        # Instead, we simulate a frequency-aware path by adding high-frequency noise 
        # that decays as t -> 1.
        
        # High frequency noise injection
        noise = torch.randn_like(x0)
        # Noise is highest at t=0.5, zero at t=0 and t=1
        freq_noise_scale = 0.5 * torch.sin(math.pi * t_expand) 
        
        xt = xt_linear + freq_noise_scale * noise
        
        # Velocity targets the linear path
        ut = x1 - x0
